...
export default class BasicLayout extends Component {
  ...
  getPageUrl = pathname => {
    ...
    // 判断是否显示另外的窗口
    const aside =
      path === '/other/company' ? newAsideMenuConfig : asideMenuConfig
    ...
    data.forEach(item => {
      if (item.path === path) {
        panes.push({
          ...
          path: bb[0].path,
          icon: item.icon || '',
          component: bb[0].component
        })
        this.setState(
          { panes, activeKey: item.key, asideData: aside },
          () => {}
        )
  ...
  remove(targetKey) {
    ...
    let panes = this.state.panes.filter(item =>
      targetKey.split('_').length - 1 > 1
        ? item.key !== targetKey
        : !(item.key.match(targetKey) || item.key === targetKey)
    )
  ...
  changeItem = type => {
    if (type === 'other') {
      const panes = this.state.panes.filter(
        item => item.key === this.state.activeKey
      )
      this.setState({ panes })
    } else {
      this.setState({ panes: [], activeKey: '' })
  ...
  render() {
    ...
    const menu = (
      <Menu>
        <Menu.Item
          disabled={panes && panes.length === 1}
          onClick={() => this.changeItem('other')}
        >
          关闭其他标签
        </Menu.Item>
        <Menu.Item onClick={() => this.changeItem('all')}>
          关闭全部标签
        </Menu.Item>
      </Menu>
    )
    const extraContent = (
      <Dropdown
        trigger={
          <FoundationSymbol
            size="large"
            type={visible ? 'angle-up' : 'angle-down'}
          />
        }
        triggerType="click"
      ...
      >
        <Layout.Section>
          <Layout.Aside width={240}>
            {asideData && asideData.length ? (
              <Aside
                asideData={asideData}
                activeKey={activeKey}
                collapse={collapse}
                toogleIsCollapse={this.toogleIsCollapse}
                    ...
                    return (
                      <Tab.Item title={item.tab} key={item.key} closeable>
                        <div className="main-container">
                          ...
                          {item.component ? (
                            <item.component itemCollapse={this.itemCollapse} />
                          ) : (
                            <NoPage />
                          )}
                          {/* </div> */}
