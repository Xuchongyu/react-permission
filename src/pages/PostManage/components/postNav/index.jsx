...
import { Input, Icon, Dialog, Switch  } from '@alifd/next';
...
export default class PostNav extends Component {
  constructor() {
    ...
    this.state = {
      value: '',
      postList: [
        { key: 'post1', post: '总经理', count: 1, type: 'show' },
        { key: 'post2', post: '副总经理', count: 10, type: 'show' },
        { key: 'post3', post: '行政专员', count: 20, type: 'show' },
        { key: 'post4', post: '人事专员', count: 2, type: 'show' },
      ],
      visible: false,
      choose: 0
    }
  ...
  deletePost = (indexOf) => {
    ...
    this.setState({ postList: postList.filter((item, index) => index !== indexOf) })
  }
  editItem = (index) => {
    const { postList } = this.state
  ...
  lookAuth = (key, index) => {
    const { lookPermissions } = this.props
  ...
  render() {
    const { value, postList, choose } = this.state
    return (
        ...
        <div className='post-nav-list'>
          {
            postList.map((item, index) => {
              if (item.type === 'edit') {
                return (
                  <div className='post-item' key={`post_${item.key}`}>
                    <div className='post-edit'>
                      <Input placeholder="请输入" value={item.post} maxLength={10}
                        onChange={(value, e) => this.changePost(value, e, index)}
                        style={{ width: '100%' }}
                      />
                    </div>
                    <div className='post-operation-edit'>
                      <Icon type="email" size='xs' onClick={() => this.savePost(index)} />
                    </div>
                  </div>
                )
              }
              return (
                <div className={`post-item ${choose === index ? 'active' : ''}`} key={`post_${item.key}`}>
                  <div className='post' onClick={() => this.lookAuth(item.key, index)}>{`${item.post}${item.count === 1 ? '' : `（${item.count}）`}`}</div>
                  <div className='post-operation'>
                    <Icon type="close" size='xs' onClick={() => this.deletePost(index)} />&nbsp;&nbsp;
                    <Icon type="edit" size='xs' onClick={() => this.editItem(index)} />
                  </div>
                </div>
              )
            })
        ...
        <Dialog
          ...
          >
            {formCore => {
              ...
              return (
                <div>
                  <Field name="name" label="名称：" labelalign='top' component={Input} placeholder="请输入名字" />
